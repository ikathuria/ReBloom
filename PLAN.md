...
- [~] `lib/db`: op-sqlite + SQLCipher — **Code complete:** interface + in-memory impl + migrations + CRUD tests; **encrypted driver `opsqlite.ts` (SQLCipher, key in Keychain via `encryptionKey.ts`) + `createDb()` wired** to use it on native and fall back to in-memory in jest/web/Expo Go. Deps: `@op-engineering/op-sqlite` 18.1.1, `expo-secure-store`, `expo-crypto`; `"op-sqlite":{"sqlcipher":true}` in package.json. **Remaining: runtime verification on a dev build** (encrypted CRUD round-trip on device).
- [~] **iOS dev build on device (early demo checkpoint):** — **Blocked on local Xcode.** This environment has Command-Line-Tools only (no Xcode/CocoaPods/simulator); user is installing full Xcode. After install: `npx expo prebuild --clean` → `npx expo run:ios` → verify onboarding→choose-tracks→garden + encrypted persistence across a restart.
