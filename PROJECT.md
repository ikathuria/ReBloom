...
_Last updated: 2026-08-21 (M0–M9 + M11 complete; M10 deploy prepped — feature-complete, pending credentialed deploy)_
...
| Fonts / type | @expo-google-fonts/fredoka + /nunito (via expo-font) | fredoka 0.4.1, nunito 0.4.2 | "Bloom Board" UI: Fredoka (rounded display) + Nunito (body); loaded at runtime in `app/_layout.tsx` (no native rebuild) |
...
│     │  └─ lib/                 # tracks/, db/, analysis/, supabase/, purchases/ (M8), telemetry/ (M10 — scrub + captureError)
│     ├─ eas.json                # EAS build + submit profiles (M10)
...
├─ supabase/
│  ├─ functions/                 # analyze-skin/, analyze-hair/, tryon-apparel/ + _shared/ (cadence.ts, report.ts)
│  └─ migrations/                # opt-in sync tables + RLS (M7)
├─ spike/                        # M0 throwaway: analyze-skin.mjs, tracks.mjs (kept as reference)
├─ docs/                         # 02-privacy-and-consent, 04-api-integration, 05-deploy ✅
├─ .github/workflows/            # ci.yml (lint/type/test) + deploy.yml (M10, tag-triggered EAS build)
...
| 10. Deploy (TestFlight / internal) | ◑ **prepped** (repo-side done; credential steps scripted) | `eas.json` (build+submit profiles), `.github/workflows/deploy.yml` (tag-triggered, `EXPO_TOKEN`-gated), `lib/telemetry` (tested image/PII scrubbing) wired into scan errors, DSN-gated Sentry in the Edge Functions (`_shared/report.ts`), Sign-in-with-Apple seam (`signInWithAppleToken`), and a full **release runbook** in `docs/05-deploy.md`. Actual TestFlight build, cloud Supabase project, RevenueCat/Apple/Sentry enablement need the user's accounts |
| 11. Polish | ✅ **done** (build-verified) | Loading + warm empty states (garden, track detail, apparel); hair error warmed to match skin; **accessibility**: `BloomVisual` is one spoken VoiceOver element (`bloomAccessibilityLabel`, tested), garden cards announce one coherent phrase + hint, sync toggle + account link labeled, dynamic-type text. Manual VoiceOver/large-type + on-device perf sweep fold into the M10 TestFlight smoke test |

**Feature-complete.** Milestones 0–9 + **M11 complete**; **M10 is prepped** — every repo-side deploy artifact is built (`eas.json` build/submit profiles, `.github/workflows/deploy.yml` tag-triggered EAS build gated on `EXPO_TOKEN`, a tested image/PII-scrubbing `lib/telemetry` seam wired into both scan error paths, DSN-gated Sentry in the Edge Functions via `_shared/report.ts`, and a Sign-in-with-Apple seam `signInWithAppleToken`), with the credential-gated steps (cloud Supabase project, RevenueCat/Apple/Sentry enablement, the TestFlight build) scripted in **`docs/05-deploy.md`**. An agent can't create accounts / enter credentials / accept Apple terms, so those are the user's to run. M11 (**polish**) added loading + warm empty states (garden, track detail, apparel), a warmed hair error, and an **accessibility** pass (`BloomVisual` speaks one VoiceOver label instead of a raw emoji; garden cards announce one coherent phrase; toggle/link labeled). M9 delivered the **privacy hardening**: **data export** (`features/privacy/exportData` → consent + enrollments + every scan point as shareable JSON, **never a photo**, via RN core `Share` so no native rebuild) and **delete everything** (`deleteAllData` → cloud rows first via `deleteCloudData`, then local `clearAll`, then sign-out), both wired into the **Account screen** and unit-tested. One shared **"not medical / not detection / not monitoring"** constant (`features/privacy/disclaimer`) now renders on consent, both scan screens (idle + result), each track's results, and the Comfort tab. The **iOS privacy manifest** + camera/photo usage strings live in `app.json` (`NSPrivacyTracking:false`; required-reason APIs `CA92.1`+`C617.1`), and the `docs/02` audit checklist is worked through. M8 added the **freemium model**: a `PurchasesProvider` seam (`lib/purchases`) hides the payment vendor exactly like `lib/analysis` hides YouCam. The app runs the **demoable mock** (a "purchase" flips a persisted flag → Pro and survives reload); the real **RevenueCat adapter is written** (dependency-injected) but its native SDK + App Store products are deferred to pre-submit (`PURCHASES_REAL=false`). Free = **one journey** (onboarding single-select + Add-a-journey routes a 2nd to `/paywall`); Pro = all journeys, full scan cadence (Free is capped monthly via `scanGate`), and the apparel try-on. A server-side **429 cadence guardrail** (`_shared/cadence.ts`) hardens the two Edge Functions.
...
**Next up:** run the **`docs/05-deploy.md` runbook** (user-credentialed): rotate the YouCam key → cloud Supabase (`link`/`db push`/`secrets set`/`functions deploy`) → enable RevenueCat (`PURCHASES_REAL=true` + install SDK + dashboard products) → Sign in with Apple → Sentry → `eas build`/`eas submit` to TestFlight → verify the privacy manifest emits → device smoke test (which also carries the M11 manual VoiceOver/large-type + performance sweep). All feature milestones are done; the remaining work is this credentialed deploy plus open vendor confirmations: YouCam **hair-density** + **apparel-VTO** endpoints (`HAIR_ANALYSIS_REAL` / `tryon-apparel`), and rotating the compromised YouCam key. Pending vendor confirmations: YouCam **hair-density** + **apparel-VTO** endpoints (flip `HAIR_ANALYSIS_REAL` / wire `tryon-apparel` once confirmed). Local dev reminder: `supabase start` + `supabase functions serve` must be running for real scans; `PERFECTCORP_API_KEY` lives in `supabase/functions/.env`. Trend charts: currently Views-based sparklines; a Skia/victory-native line chart is a deferred polish (needs a native rebuild).
...
- 2026-08-21 — **Polish (M11): accessibility centers on the bloom.** The garden's meaning lives in a decorative emoji + number, which a screen reader would read as a raw emoji name — so `BloomVisual` became a single VoiceOver element with a spoken label (`bloomAccessibilityLabel`, e.g. "blooming, 80 out of 100"), and each garden card overrides to one coherent phrase (name + stage + last-scan) rather than announcing its children piecemeal. Loading/empty states were added where a screen could briefly render nothing or flash a wrong empty state (garden while enrollments load + after a full delete; track detail before points load; apparel before the latest scan resolves). The manual VoiceOver/large-type + on-device frame sweep can't run here (no Simulator panel) and is folded into the M10 device smoke test — code-level a11y is done, the human sweep is pending a real build.
- 2026-08-21 — **Deploy (M10) is prepped repo-side, executed by the user.** The milestone's "done" gates (a TestFlight build, an error in Sentry, a device smoke test) all require accounts/credentials an agent can't create or enter, so M10 splits cleanly: **built now** = `eas.json`, `deploy.yml` (tag-triggered, `EXPO_TOKEN`-gated), the `lib/telemetry` scrub-then-report seam (images/PII redacted before any reporter, unit-tested) wired into the scan error paths, DSN-gated `_shared/report.ts` in the Edge Functions (forwards only a tag + message, never a body/image), and the `signInWithAppleToken` seam; **scripted for the user** = the ordered command runbook in `docs/05-deploy.md`. Telemetry is **no-op until a reporter is registered** — nothing leaves the device by default, consistent with the privacy stance. Client Sentry + RevenueCat + Apple-auth native packages stay uninstalled (would need a rebuild) and enable via one-liners at deploy — same deferral pattern as M8's `PURCHASES_REAL`.
...
- 2026-08-21 — **UI redesign: "Bloom Board" — a Pinterest-inspired, warm Gen-Z direction.** The neutral template palette + lone green accent gave way to a design-token system in `constants/theme.ts`: a warm paper ground, the hopeful green kept as the soul (`bloom`/`bloomDeep`), a coral pop accent, and a set of soft pastel **bloom hues** (`BloomHues` light+dark) with one distinct hue assigned per track (`hueFor`) + an identity emoji (`emojiFor`) — the colorful "board" energy. Type moved from `system-ui` to **Fredoka** (rounded display, headings + scores) + **Nunito** (body), loaded at runtime via `@expo-google-fonts` in `app/_layout.tsx` (splash held until fonts ready; no native rebuild). Added `Radius` + `softShadow` tokens (chunky rounded cards, pill CTAs). The garden home became a **two-column masonry board** of hue-colored journey cards; track detail got a colored hero bloom card; onboarding/add-journey use sticker chips + progress dots; `PrimaryButton` is a bloom pill. All colors are theme-token-driven (light+dark), score pills/progress chrome made contrast-safe on the deep dark-mode pastels. Verified: typecheck + lint + **68** unit tests green; **no live render** — Expo web fails on op-sqlite's `better-sqlite3` node fallback and the iOS Simulator panel is unavailable here, so on-device visual QA folds into the M10 device smoke test.
- 2026-08-21 — **Selectable "garden styles" (skins) so the look isn't gender-locked.** The floral "Bloom Board" identity skewed femme-coded — a barrier for the large male audience of the Hair Regrowth / Acne / Recovery tracks. Rather than a single look or a "boy/girl mode," ReBloom ships **three user-selectable skins** (`lib/skins`): **Blooms** (soft florals, the default — nothing changes for existing users), **Grove** (earthy trees/foliage, plant-parent neutral), and **Zen** (quiet stone & moss, calm low-saturation minimal). A skin swaps *presentation only* — the growth-stage emojis + per-track card hues — never scores, tracks, copy, or privacy. State lives in a `SkinProvider` context (`useSkin()`), persisted via the existing `db.getFlag/setFlag('skin')` (no schema change), defaulting to Blooms with a provider-less fallback so unit tests need no wrapper. Chosen in **Account → Garden style**. A registry test asserts every skin covers all 7 tracks + 6 stages. **A 4th skin, `custom`, is a Pro perk**: the user picks their own emoji for each of the 6 growth stages from a curated palette (colors inherit from Grove, so only the emojis are personalized). Gated by `canCustomizeEmojis(tier)` (free → paywall, like try-on); choices persist as JSON in `db.setFlag('skin.custom.emoji')` and merge into the live skin via `SkinProvider`. The emoji editor lives under the picker in the Account screen — a curated palette **and** a free-text "type any emoji" field, backed by a grapheme-safe `firstEmoji()` (Unicode property escapes with an `Intl.Segmenter`/code-point fallback; keeps skin tones, ❤️-style variation selectors, and ZWJ sequences intact; unit-tested). Verified: typecheck + lint + **83** tests green (still no live render here — same env limits as the redesign entry below).
