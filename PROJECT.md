...
**In progress now:** Milestones 0–7 **complete and verified on iOS**. M7 added **email auth** (session in the device Keychain) + an **Account & sync** screen with **opt-in cloud sync** — enrollments + track points (never images) push to per-user cloud tables under **owner-only RLS** (verified: owner sees 5, other user sees 0). The app stays fully usable logged-out. Backend still runs on local Supabase (Docker).

_Earlier context:_ M6 = Comfort tab (fabric suggestions). M5 = Hair Regrowth. M4 = the garden. M0–M3 = spike, scaffold, consent+encrypted store, real skin scan. M3 closed with a **real** end-to-end scan on iPhone 17 Pro: selfie → `analyze-skin` Edge Function (local Supabase, Docker) → real YouCam scores → fan-out to blooms (Recovery 80 / Acne 78), API key server-side. The app auto-selects the real provider when `EXPO_PUBLIC_SUPABASE_URL` is set, else the mock. Local backend runs via `supabase start` + `supabase functions serve` (key in gitignored `supabase/functions/.env`).
**Next up:** Milestone 8 — Monetization (RevenueCat): free = 1 track at capped cadence; Pro = all tracks + full cadence + the apparel VTO try-on; server-side cadence/track gating in the Edge Functions. Then M9 privacy hardening (data export/delete, privacy manifest), M10 deploy (deployed cloud Supabase project + `secrets set`/`functions deploy`/`db push`, EAS/TestFlight, Sign in with Apple), M11 polish. Pending vendor confirmations: YouCam **hair-density** + **apparel-VTO** endpoints (flip `HAIR_ANALYSIS_REAL` / wire `tryon-apparel` once confirmed). Local dev reminder: `supabase start` + `supabase functions serve` must be running for real scans; `PERFECTCORP_API_KEY` lives in `supabase/functions/.env`. Trend charts: currently Views-based sparklines; a Skia/victory-native line chart is a deferred polish (needs a native rebuild).
