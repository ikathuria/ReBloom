...
_Last updated: 2026-08-21 (M0–M8 complete; M9 privacy hardening next)_
...
**In progress now:** Milestones 0–8 **complete** (0–7 verified live on iOS; M8 build-verified — see note below). M8 added the **freemium model**: a `PurchasesProvider` seam (`lib/purchases`) hides the payment vendor exactly like `lib/analysis` hides YouCam. The app runs the **demoable mock** (a "purchase" flips a persisted flag → Pro and survives reload); the real **RevenueCat adapter is written** (dependency-injected) but its native SDK + App Store products are deferred to pre-submit (`PURCHASES_REAL=false`). Free = **one journey** (onboarding single-select + Add-a-journey routes a 2nd to `/paywall`); Pro = all journeys, full scan cadence (Free is capped monthly via `scanGate`), and the apparel try-on. A server-side **429 cadence guardrail** (`_shared/cadence.ts`) hardens the two Edge Functions.

⚠️ **M8 verification note:** verified by typecheck + lint + **56 unit tests** + a clean full **iOS Metro bundle build** (every import resolves) + app relaunch on device. Live **screenshots were not captured** — the Simulator control panel hit a crash-loop it can't self-recover from (needs the user to re-open it). No visual walkthrough was recorded for M8.
...
**Next up:** Milestone 9 — Privacy & compliance hardening (data export + delete, iOS privacy manifest + SDK declarations, "not medical/not detection/not monitoring" disclaimers on every scan surface, the Loosid/Sober-Grid contrast audit). Then M10 deploy (deployed cloud Supabase project + `secrets set`/`functions deploy`/`db push`, EAS/TestFlight, Sign in with Apple, **flip `PURCHASES_REAL` + install `react-native-purchases` + RC dashboard products**), M11 polish. Pending vendor confirmations: YouCam **hair-density** + **apparel-VTO** endpoints (flip `HAIR_ANALYSIS_REAL` / wire `tryon-apparel` once confirmed). Local dev reminder: `supabase start` + `supabase functions serve` must be running for real scans; `PERFECTCORP_API_KEY` lives in `supabase/functions/.env`. Trend charts: currently Views-based sparklines; a Skia/victory-native line chart is a deferred polish (needs a native rebuild).
